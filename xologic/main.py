class Logic():
    ...
    def vertical(self, board):
        at = 0
        for i in range(self.board_size):
            number_indexes = [i+at for i in range(0, self.board_size**2, self.board_size)]
    ...
    def diagonal_2(self, board):
        number_indexes = [i for i in range(self.board_size-1, self.board_size**2, self.board_size-1)][:self.board_size]
        # print(number_indexes)
        pos = map(lambda x:board[x], number_indexes)
